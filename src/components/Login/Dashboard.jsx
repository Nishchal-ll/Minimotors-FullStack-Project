...
// import { FaUser, FaShoppingBag, FaSignOutAlt, FaBox, FaClock, FaCheckCircle, FaTruck, FaTimesCircle, FaTrash, FaArrowLeft } from 'react-icons/fa';
// import { MdEmail, MdShoppingCart } from 'react-icons/md';
// import SubmitReview from "../Login/SiteReview";
...
//         </div>
//       </div>
//       <div className="max-w-2xl mx-auto bg-white p-6 mt-12 rounded-xl shadow-md border">
// <h2 className="text-2xl font-bold mb-2 text-gray-800">Leave us a Review</h2>
// <p className="text-gray-600 mb-4">We value your feedback to improve your shopping experience.</p>
// <SubmitReview />
// </div>
//       <Footer />
//     </>
...
// ✅ Inline Review Form
function SubmitSiteReview({ user }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col space-y-2 max-w-lg">
      <input type="text" placeholder="Your Name" value={name} onChange={(e) => setName(e.target.value)} required className="border p-2 rounded" />
      <input type="email" placeholder="Your Email" value={email} onChange={(e) => setEmail(e.target.value)} required className="border p-2 rounded" />
      <textarea placeholder="Write your review..." value={review} onChange={(e) => setReview(e.target.value)} required className="border p-2 rounded" />
      <input type="number" min="1" max="5" value={rating} onChange={(e) => setRating(e.target.value)} className="border p-1 w-20" />
...
const Dashboard = () => {
  ...
  const getStatusStyle = (status) => {
    switch(status?.toLowerCase()) {
      case 'pending': return { color: 'text-yellow-600', bg: 'bg-yellow-50', icon: <FaClock /> };
      case 'processing': return { color: 'text-blue-600', bg: 'bg-blue-50', icon: <FaTruck /> };
      case 'completed': return { color: 'text-green-600', bg: 'bg-green-50', icon: <FaCheckCircle /> };
      case 'cancelled': return { color: 'text-red-600', bg: 'bg-red-50', icon: <FaTimesCircle /> };
      default: return { color: 'text-gray-600', bg: 'bg-gray-50', icon: <FaClock /> };
    }
  };
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-50 pt-32 pb-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-lg border p-8 mb-8 flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="flex gap-3 mt-4 md:mt-0">
              <button onClick={() => navigate('/shop')} className="px-6 py-3 bg-blue-600 text-white rounded-lg flex items-center gap-2"><FaArrowLeft/>Back to Shop</button>
              <button onClick={handleLogout} className="px-6 py-3 bg-red-600 text-white rounded-lg flex items-center gap-2"><FaSignOutAlt/>Logout</button>
            </div>
          </div>
          {/* Stats */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <StatCard title="Cart Items" value={cartItems.length} icon={<MdShoppingCart/>} color="purple" />
            <StatCard title="Total Orders" value={orders.length} icon={<FaShoppingBag/>} color="blue" />
            <StatCard title="Pending Orders" value={orders.filter(o=>o.status?.toLowerCase()==='pending').length} icon={<FaClock/>} color="yellow" />
            <StatCard title="Completed Orders" value={orders.filter(o=>o.status?.toLowerCase()==='completed').length} icon={<FaCheckCircle/>} color="green" />
          </div>
        </div>
...
function StatCard({ title, value, icon, color }) {
  return (
    <div className="bg-white rounded-xl shadow-md border p-6 flex justify-between items-center">
      <div>
        <p className="text-gray-600 text-sm font-medium">{title}</p>
        <p className="text-3xl font-bold text-gray-900 mt-1">{value}</p>
      </div>
      <div className={`w-12 h-12 bg-${color}-100 rounded-full flex items-center justify-center`}>{icon}</div>
    </div>
  );
}